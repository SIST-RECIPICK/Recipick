...
public class RefridgeServiceImpl implements RefridgeService {
	...
	public List<Map<String, Object>> recommandRecipe(List<String> ingredients) {
		...
		// PostgreSQL pgVector 유사 검색

		List<Map<String, Object>> recipes = prMapper.findSimilarRecipes(vectorString, 7);
		...
		for (Map<String, Object> recipe : recipes) {
			...
			List<String> recipeIngredients = extractIngredients(content);

			// 사용자가 가지고 있는 재료와 레시피 재료 비교
	 ...
	 *
	 * 주재료: 새송이버섯 100g 두유 20g 생크림 10g
	 *
	 * 형태이므로 "주재료" 이후 내용을 분석한다.
	 *
	 * 정확한 DB 포맷이 더 세분화되어 있다면 이 부분만 수정하면 된다.
	 */
...
//	        };
		 ...
		 *
		 * 프로젝트에서 사용하는 실제 재료 목록을 이 배열에 추가하면 된다.
		 */
//	        for (String ingredient : searchData) {
//
...
//	    }
	...
	private String normalizeIngredient(String ingredient) {
		...
		return ingredient.replaceAll("[0-9]+(\\.\\d+)?", "").replaceAll("(g|kg|ml|L|개|모|대|큰술|작은술|컵|쪽|약간)", "")
	...
	private List<String> extractCookingSteps(String content) {
		...
		for (String sentence : sentences) {
			 ...
			 * 너무 짧은 문장 제외
			 */
			if (value.length() >= 5 && !value.startsWith("레시피명:") && !value.startsWith("영양정보:")
					&& !value.startsWith("탄수화물:") && !value.startsWith("단백질:") && !value.startsWith("지방:")
					&& !value.startsWith("나트륨:") && !value.startsWith("해시태그:") && !value.startsWith("주재료:")) {
